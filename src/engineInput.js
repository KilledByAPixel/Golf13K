...
{
    if (!glCanvas || headlessMode)
        return vec2(); // fix bug that can occur if user clicks before page loads

    const rect = glCanvas.getBoundingClientRect();
    return vec2(glCanvas.width, glCanvas.height).multiply(
...
function gamepadsUpdate()
{
    ...
    // poll gamepads
    // (getGamepads can throw when blocked by a permissions policy,
    // e.g. sandboxed iframes - treat as gamepads being unsupported)
    let gamepads;
    try { gamepads = navigator.getGamepads(); }
    catch { return; }
