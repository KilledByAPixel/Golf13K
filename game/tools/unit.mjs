...
const src =
    ...
    // Wind is rolled per PLAY from Math.random now, so seed it (the same LCG
    // as sim.mjs --fixed) - otherwise every run draws a different wind and
    // any case that depends on it is flaky.
    ...
    // the harness supplies one - packColor dots every normal against it.
    // The game itself has no default: buildWorld always runs before anything
    // is pushed, and a dead initialiser would only cost release bytes.
